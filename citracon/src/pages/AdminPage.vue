<template>
  <q-layout view="lHh Lpr lFf">
    <q-page-container>
      <q-page
        class="min-h-screen py-10 px-4 sm:px-8 relative overflow-hidden"
        style="background: linear-gradient(135deg, #080f20 0%, #0b1a38 50%, #0e1f42 100%)"
      >
        <!-- Ambient Lights -->
        <div
          class="absolute top-[-10%] left-[-5%] w-[600px] h-[600px] rounded-full pointer-events-none"
          style="background: radial-gradient(circle, rgba(210, 29, 29, 0.12) 0%, transparent 70%)"
        ></div>
        <div
          class="absolute bottom-[-10%] right-[-5%] w-[500px] h-[500px] rounded-full pointer-events-none"
          style="background: radial-gradient(circle, rgba(59, 130, 246, 0.1) 0%, transparent 70%)"
        ></div>
        <div
          class="absolute top-[40%] right-[20%] w-[300px] h-[300px] rounded-full pointer-events-none"
        ...
        <div class="max-w-7xl mx-auto relative z-10">
          <!-- HEADER -->
          <div
            class="flex flex-col justify-center items-center gap-4 mb-10 pb-6 border-b border-white/10 relative text-center"
          >
            <div class="w-full">
              <div class="flex flex-col items-center justify-center text-center">
                <div
                  class="inline-flex items-center gap-2 bg-red-500/10 border border-red-500/30 rounded-full px-4 py-1.5 mb-3"
                >
                ...
                <p class="text-sm text-slate-400 mt-2">
                  Kelola katalog produk dan pantau pesanan masuk secara real-time.
                </p>
              </div>
            </div>
          ...
          <div v-if="!isAuthenticated" class="max-w-md mx-auto my-12">
            ...
            >
              <div class="text-center mb-6">
                <div
                  class="w-16 h-16 mx-auto rounded-2xl flex items-center justify-center text-3xl shadow-2xl mb-4"
                  style="background: linear-gradient(135deg, #d21d1d, #ff6b6b)"
                >
                  🔑
                </div>
              ...
              <div class="space-y-4">
                ...
                <button
                  ...
                  style="
                    background: linear-gradient(135deg, #d21d1d, #b21818);
                    box-shadow: 0 8px 24px rgba(210, 29, 29, 0.35);
                  "
                >
                  Masuk Dasbor 🚀
                </button>
          ...
          <div v-else>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-10">
              <div
                class="premium-glow-card glow-blue rounded-2xl p-5 flex items-center gap-4 border cursor-pointer"
              ...
              >
                ...
                <div>
                  <div class="text-2xl font-black" style="color: #0b1c3f">
                    {{ products.length }}
                  </div>
                  <div class="text-xs font-bold mt-0.5 text-slate-500">Total Produk Katalog</div>
                </div>
                <div
                  class="ml-auto w-1 h-12 rounded-full"
                  style="background: linear-gradient(180deg, #3b82f6, rgba(59, 130, 246, 0))"
                ></div>
              </div>
              <div
                class="premium-glow-card glow-green rounded-2xl p-5 flex items-center gap-4 border cursor-pointer"
                style="background: #ffffff; border-color: rgba(0, 0, 0, 0.08)"
              >
                <div
                  class="w-12 h-12 rounded-xl flex items-center justify-center text-2xl"
                ...
                <div
                  class="ml-auto w-1 h-12 rounded-full"
                  style="background: linear-gradient(180deg, #10b981, rgba(16, 185, 129, 0))"
                ></div>
              </div>
              <div
                class="premium-glow-card glow-red rounded-2xl p-5 flex items-center gap-4 border cursor-pointer"
                style="background: #ffffff; border-color: rgba(0, 0, 0, 0.08)"
              >
                <div
                  class="w-12 h-12 rounded-xl flex items-center justify-center text-2xl"
                  style="
                    background: rgba(210, 29, 29, 0.08);
                    border: 1px solid rgba(210, 29, 29, 0.15);
                  "
                >
                  🔥
                </div>
                <div>
                  <div class="text-2xl font-black" style="color: #0b1c3f">
                    {{ activeTab === 'leads' ? leads.length : products.length }}
                  </div>
                  <div class="text-xs font-bold mt-0.5 text-slate-500">Item Sedang Dikelola</div>
                </div>
                <div
                  class="ml-auto w-1 h-12 rounded-full"
                  style="background: linear-gradient(180deg, #d21d1d, rgba(210, 29, 29, 0))"
                ></div>
              </div>
            </div>
            ...
            <!-- TABS HEADER -->
            <div
              class="flex gap-1.5 p-1.5 rounded-2xl max-w-lg mb-8"
              style="
                background: rgba(255, 255, 255, 0.07);
            ...
            >
              <button
                ...
                :class="
                  activeTab === 'leads'
                    ? 'bg-[#d21d1d] text-white shadow-lg'
            ...
            <div
              ...
              class="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm shadow-slate-100"
            >
              <div class="flex justify-between items-center mb-6">
                <div>
                  <h3 class="text-lg font-extrabold text-slate-900 flex items-center gap-2">
                    <span
                      class="w-8 h-8 rounded-lg bg-emerald-50 border border-emerald-100 flex items-center justify-center text-base"
                      >📨</span
                    >
                    Daftar Konsultasi / Pesanan
                  </h3>
                  <p class="text-xs text-slate-400 mt-1 ml-10">
                    Data masuk dari form konsultasi website
                  </p>
                </div>
                <button
                  @click="fetchLeads"
              ...
              <div v-if="leads.length === 0" class="text-center py-16">
                ...
                <p class="text-xs text-slate-400 mt-1">
                  Data akan muncul ketika ada yang mengisi form konsultasi
                </p>
              </div>
                ...
                <table class="w-full border-collapse text-left text-xs sm:text-sm">
                  <thead>
                    <tr class="border-b border-slate-200 bg-slate-50">
                      ...
                      <th
                        class="p-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider"
                      >
                        Aksi
                      </th>
                    </tr>
                  ...
                  <tbody class="divide-y divide-slate-100">
                    ...
                    >
                      <td class="p-4 text-xs text-slate-400 whitespace-nowrap font-medium">
                      ...
                      <td class="p-4">
                        <span
                          class="px-2.5 py-1 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100 text-xs font-semibold"
                        >
                          {{ lead.product_name || 'Umum' }}
                        </span>
                      </td>
                      <td
                        class="p-4 max-w-xs truncate text-xs text-slate-500 font-medium"
                        :title="lead.address"
                      >
                        {{ lead.address }}
                      </td>
            ...
            >
              ...
              >
                <div>
                  <h3 class="text-lg font-extrabold text-slate-900 flex items-center gap-2">
                    <span
                      class="w-8 h-8 rounded-lg bg-blue-50 border border-blue-100 flex items-center justify-center text-base"
                      >📦</span
                    >
                    Daftar Katalog Produk
                  </h3>
                  <p class="text-xs text-slate-400 mt-1 ml-10">
                    Kelola semua produk precast yang ditampilkan di website
                  </p>
                </div>
                <button
              ...
              <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                >
                  <div>
                    <div
                      class="h-44 overflow-hidden relative bg-gradient-to-br from-slate-100 to-slate-200"
                    >
                      <img
                        :src="p.image"
                      ...
                      ></div>
                      <div
                        class="absolute top-3 left-3 w-9 h-9 rounded-xl bg-white shadow-md flex items-center justify-center text-xl"
                      >
                        {{ p.icon || '📦' }}
                      </div>
                      <div
                        class="absolute bottom-3 right-3 px-2.5 py-1 rounded-lg bg-black/50 backdrop-blur-sm text-xs font-bold text-white"
                      >
                        ID #{{ p.id }}
                      </div>
                    </div>
                    ...
                    <div class="p-4">
                      ...
                      <div
                        class="inline-block text-[11px] text-[#d21d1d] font-bold mt-1 bg-red-50 border border-red-100 px-2 py-0.5 rounded-md"
                      >
                        {{ p.specs }}
                      </div>
                      <p class="text-xs text-slate-500 line-clamp-2 mt-2.5 leading-relaxed">
                        {{ p.detailed_desc }}
                      </p>
                    </div>
                  </div>
            ...
            <div
              ...
              class="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm shadow-slate-100 space-y-8"
            >
              <div
                class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-5 border-b border-slate-100"
              >
                <div>
                  <h3 class="text-lg font-extrabold text-slate-900 flex items-center gap-2">
                    <span
                      class="w-8 h-8 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center text-base"
                      >⚙️</span
                    >
                    Pengaturan Tampilan & Konten
                  </h3>
                  <p class="text-xs text-slate-400 mt-1 ml-10">
                    Ubah konten website secara langsung dari sini
                  </p>
                </div>
                <button
              ...
              <div class="space-y-4">
                <div class="flex justify-between items-center">
                  <h4 class="text-sm font-bold text-[#d21d1d] uppercase tracking-wider">
                    1. Banner Slides (Slider Hero Depan)
                  </h4>
                  <div class="flex gap-2">
                    <button
                      @click="addSlide"
                      class="text-xs bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-1.5 rounded-lg border border-slate-250 cursor-pointer"
                    >
                      ➕ Tambah Slide
                    </button>
                    <button
                      @click="saveSiteSettings"
                      :disabled="isSavingSettings"
                      class="text-xs bg-emerald-600 hover:bg-emerald-700 text-white px-3.5 py-1.5 rounded-lg border-0 cursor-pointer font-bold transition-all shadow-sm flex items-center gap-1"
                    >
                      <span>💾 Simpan</span>
                    </button>
                  </div>
                ...
                <div class="grid grid-cols-1 gap-6">
                  <div
                    v-for="(slide, sIdx) in siteSettings.hero_slides"
                    :key="sIdx"
                    class="rounded-2xl p-4 relative"
                    style="background: #0b1c3f; border: 1px solid rgba(255, 255, 255, 0.12)"
                  >
                    <button
                      @click="removeSlide(sIdx)"
                      class="absolute top-4 right-4 bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/30 px-2 py-1 rounded text-xs cursor-pointer"
                    >
                      Hapus
                    </button>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label
                          ...
                          >Gambar Banner (Latar Belakang)</label
                        >
                        <div class="flex gap-2">
                          <input
                            v-model="slide.image"
                            type="text"
                            class="flex-grow rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                            style="
                              background: rgba(255, 255, 255, 0.08);
                              border: 1px solid rgba(255, 255, 255, 0.15);
                            "
                            placeholder="Contoh: hero-precast-1.png atau URL eksternal"
                          />
                          <label
                            class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-xl text-xs font-bold cursor-pointer transition-all flex items-center justify-center gap-1 shrink-0 border-0"
                          >
                            <span>📁 Upload</span>
                            <input
                              type="file"
                              class="hidden"
                              accept="image/*"
                              @change="(e) => uploadSlideImage(e, sIdx)"
                            />
                          </label>
                        </div>
                      </div>
                      <div>
                        <label
                          class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                          >Judul Banner (Mendukung HTML)</label
                        >
                        <input
                          v-model="slide.title"
                          type="text"
                          class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                          style="
                            background: rgba(255, 255, 255, 0.08);
                            border: 1px solid rgba(255, 255, 255, 0.15);
                          "
                          placeholder="Contoh: MEMBANGUN FONDASI..."
                        />
                      </div>
                      <div class="md:col-span-2">
                        <label
                          class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                          >Deskripsi Pendek</label
                        >
                        <textarea
                          v-model="slide.desc"
                          rows="2"
                          class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none resize-none"
                          style="
                            background: rgba(255, 255, 255, 0.08);
                            border: 1px solid rgba(255, 255, 255, 0.15);
                          "
                          placeholder="Tulis deskripsi singkat slide..."
                        ></textarea>
                      </div>
                    </div>
                  </div>
                ...
                >
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2">
                      ...
                      >
                      <input
                        v-model="siteSettings.profile_title"
                        type="text"
                        class="w-full rounded-xl px-3 py-2.5 text-white text-xs focus:outline-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                          border: 1px solid rgba(255, 255, 255, 0.15);
                        "
                    ...
                    <div>
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >Paragraf Deskripsi 1 (Mendukung HTML)</label
                      >
                      <textarea
                        v-model="siteSettings.profile_desc_1"
                        rows="4"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none resize-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                    ...
                    <div>
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >Paragraf Deskripsi 2 (Mendukung HTML)</label
                      >
                      <textarea
                        v-model="siteSettings.profile_desc_2"
                        rows="4"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none resize-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                    ...
                    <div class="md:col-span-2">
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >Pernyataan Visi</label
                      >
                      <textarea
                        v-model="siteSettings.visi_text"
                        rows="3"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none resize-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                    ...
                    <div class="md:col-span-2 space-y-2">
                      <div class="flex justify-between items-center">
                        <label
                          ...
                          >Butir-Butir Misi Kami</label
                        >
                        <button
                          @click="addMisi"
                          class="text-[11px] bg-slate-800 hover:bg-slate-900 text-white px-3 py-1.5 rounded-xl font-bold border-0 cursor-pointer transition-all shadow-sm"
                        >
                          ➕ Tambah Misi
                        </button>
                      </div>
                      <div class="space-y-2">
                        <div
                          v-for="(misi, mIdx) in siteSettings.misi_items"
                          :key="mIdx"
                          class="flex gap-2 items-center"
                        >
                          <span class="text-xs text-slate-400 font-bold">#{{ mIdx + 1 }}</span>
                          <input
                            v-model="siteSettings.misi_items[mIdx]"
                            type="text"
                            class="flex-grow rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                            style="
                              background: rgba(255, 255, 255, 0.08);
                          ...
                          />
                          <button
                            @click="removeMisi(mIdx)"
                            class="bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/30 px-2.5 py-2 rounded-xl text-xs cursor-pointer"
                          >
                            ✕
                          </button>
                        </div>
              ...
              <div class="pt-6 border-t border-white/10 space-y-4">
                <div class="flex justify-between items-center">
                  <h4 class="text-sm font-bold text-[#ef4444] uppercase tracking-wider">
                    3. Portofolio Proyek (Galeri Kerja)
                  </h4>
                  <div class="flex gap-2">
                    <button
                      @click="addPortfolioItem"
                      class="text-xs bg-slate-800 hover:bg-slate-900 text-white px-4 py-2 rounded-xl font-bold border-0 cursor-pointer transition-all shadow-sm"
                    >
                      ➕ Tambah Proyek
                    </button>
                    <button
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div
                    v-for="(item, pIdx) in siteSettings.portfolio_items"
                    :key="pIdx"
                    class="rounded-2xl p-4 relative space-y-3"
                    style="background: #0b1c3f; border: 1px solid rgba(255, 255, 255, 0.12)"
                  >
                    <button
                      @click="removePortfolioItem(pIdx)"
                      class="absolute top-4 right-4 bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/30 px-2 py-1 rounded text-xs cursor-pointer"
                    >
                      Hapus
                    </button>
                    ...
                    <div class="grid grid-cols-1 gap-2.5">
                      <div>
                        ...
                        <input
                          ...
                          type="text"
                          class="w-full rounded-xl px-3 py-1.5 text-white text-xs focus:outline-none"
                          style="
                            background: rgba(255, 255, 255, 0.08);
                            border: 1px solid rgba(255, 255, 255, 0.15);
                          "
                          placeholder="Contoh: Jalan & Drainase"
                        />
                      </div>
                      <div>
                        <label
                        ...
                        <input
                          ...
                          style="
                            ...
                            border: 1px solid rgba(255, 255, 255, 0.15);
                          "
                          placeholder="Contoh: Drainase Tol Trans-Jawa"
                        />
                      </div>
                      <div>
                        <label
                        ...
                        <input
                          ...
                          style="
                            ...
                            border: 1px solid rgba(255, 255, 255, 0.15);
                          "
                          placeholder="Detail singkat pengadaan..."
                        />
                      </div>
                      <div>
                        <label
                        ...
                        <div class="flex gap-2">
                          <input
                            v-model="item.image"
                            type="text"
                            class="flex-grow rounded-xl px-3 py-1.5 text-white text-xs focus:outline-none"
                            style="
                              background: rgba(255, 255, 255, 0.08);
                              border: 1px solid rgba(255, 255, 255, 0.15);
                            "
                          ...
                          />
                          <label
                            class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-xl text-xs font-bold cursor-pointer transition-all flex items-center justify-center gap-1 shrink-0 border-0"
                          >
                            <span>📁 Upload</span>
                            <input
                              type="file"
                              class="hidden"
                              accept="image/*"
                              @change="(e) => uploadPortfolioImage(e, pIdx)"
                            />
                          </label>
                        </div>
                      </div>
                ...
                >
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >Sub-judul Maskot (Samping "MEMPERKENALKAN TRACON")</label
                      >
                      <input
                        v-model="siteSettings.tracon_subtitle"
                        type="text"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                    ...
                    <div>
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >Judul Zona Maskot (Beranda - Home)</label
                      >
                      <input
                        v-model="siteSettings.tracon_title"
                        type="text"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                    ...
                    <div class="md:col-span-2">
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >Deskripsi Utama Maskot (Beranda - Home)</label
                      >
                      <textarea
                        v-model="siteSettings.tracon_desc"
                        rows="2"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none resize-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                  ...
                  <div class="space-y-2 border-t border-white/10 pt-4">
                    <label
                      class="block text-[10px] font-bold uppercase tracking-wider text-slate-400"
                      >3 Kartu Informasi Ringkas (Kekuatan, Keramahan, Kualitas)</label
                    >
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div
                        v-for="(stat, sIdx) in siteSettings.tracon_fun_stats"
                        :key="sIdx"
                      ...
                      >
                        <div class="flex gap-2">
                          <input
                            v-model="stat.icon"
                            type="text"
                            class="w-10 rounded px-1 py-1 text-white text-center text-xs focus:outline-none"
                            style="
                              background: rgba(255, 255, 255, 0.08);
                              border: 1px solid rgba(255, 255, 255, 0.15);
                            "
                          ...
                          <input
                            v-model="stat.title"
                            type="text"
                            class="flex-grow rounded px-2 py-1 text-white text-xs font-bold focus:outline-none"
                            style="
                              background: rgba(255, 255, 255, 0.08);
                              border: 1px solid rgba(255, 255, 255, 0.15);
                            "
                        ...
                        <input
                          ...
                          type="text"
                          class="w-full rounded px-2 py-1 text-white text-[11px] focus:outline-none"
                          style="
                            background: rgba(255, 255, 255, 0.08);
                            border: 1px solid rgba(255, 255, 255, 0.15);
                          "
                  ...
                  <div class="space-y-2 border-t border-white/10 pt-4">
                    <div class="flex justify-between items-center">
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400"
                        >Daftar Obrolan / Gelembung Teks Maskot</label
                      >
                      <button
                    ...
                    <div class="space-y-2">
                      <div
                        v-for="(bubble, bIdx) in siteSettings.tracon_speech_bubbles"
                        :key="bIdx"
                        class="flex gap-2 items-center"
                      >
                        <span class="text-xs text-slate-400 font-bold">#{{ bIdx + 1 }}</span>
                        <input
                          v-model="siteSettings.tracon_speech_bubbles[bIdx]"
                          type="text"
                          class="flex-grow rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                          style="
                            background: rgba(255, 255, 255, 0.08);
                  ...
                  <div class="space-y-2 border-t border-white/10 pt-4">
                    <div class="flex justify-between items-center">
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400"
                        >Daftar Pertanyaan & Jawaban Chatbot FAQ</label
                      >
                      <div class="flex gap-2">
                        <button
                          @click="resetFaqToDefault"
                          class="text-[11px] bg-red-500/20 hover:bg-red-500/40 text-red-300 px-3 py-1.5 rounded-xl font-bold border border-red-500/30 cursor-pointer transition-all shadow-sm"
                        >
                          🔄 Reset ke Default
                        </button>
                        <button
                    ...
                    <div class="space-y-4">
                      <div
                        v-for="(faq, fIdx) in siteSettings.tracon_faq"
                        :key="fIdx"
                        class="p-3 rounded-xl relative space-y-2"
                        style="
                          background: rgba(255, 255, 255, 0.06);
                          border: 1px solid rgba(255, 255, 255, 0.12);
                        "
                      >
                        <button
                          @click="removeFaqItem(fIdx)"
                          class="absolute top-2 right-2 bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/30 px-2 py-0.5 rounded text-[10px] cursor-pointer"
                        >
                          ✕ Hapus
                        </button>
                        ...
                        <div>
                          <label
                            class="block text-[9px] font-bold uppercase tracking-wider text-slate-500 mb-1"
                            >Pertanyaan</label
                          >
                          <input
                            v-model="faq.question"
                            type="text"
                            class="w-11/12 rounded px-2 py-1 text-white text-xs font-bold focus:outline-none"
                            style="
                              background: rgba(255, 255, 255, 0.08);
                              border: 1px solid rgba(255, 255, 255, 0.15);
                            "
                            placeholder="Contoh: Berapa minimal order?"
                          />
                        </div>
                        <div>
                          <label
                            class="block text-[9px] font-bold uppercase tracking-wider text-slate-500 mb-1"
                            >Jawaban Otomatis TRACON</label
                          >
                          <textarea
                            v-model="faq.answer"
                            rows="3"
                            class="w-full rounded px-2 py-1 text-white text-[11px] focus:outline-none resize-none"
                            style="
                              background: rgba(255, 255, 255, 0.08);
                              border: 1px solid rgba(255, 255, 255, 0.15);
                            "
                            placeholder="Tulis jawaban detail dari TRACON..."
                          ></textarea>
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="space-y-2 border-t border-white/10 pt-4">
                    <div class="flex justify-between items-center">
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400"
                        >Foto / Slide Maskot TRACON (Mendukung Multi-Foto)</label
                      >
                      <label
                        class="text-[10px] bg-slate-800 hover:bg-slate-900 text-white px-3 py-2 rounded-xl font-bold border-0 cursor-pointer transition-all shadow-sm flex items-center gap-1"
                      >
                        ...
                        <input
                          ...
                          class="hidden"
                          accept="image/*"
                          @change="uploadMascotImage"
                        />
                      </label>
                    </div>

                    <div
                      v-if="siteSettings.tracon_images && siteSettings.tracon_images.length"
                      class="grid grid-cols-2 sm:grid-cols-5 gap-4"
                    >
                      <div
                        v-for="(imgUrl, idx) in siteSettings.tracon_images"
                        :key="idx"
                      ...
                      >
                        ...
                        >
                          <button
                            @click="removeMascotImage(idx)"
                            class="bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/30 p-1.5 rounded-lg text-xs cursor-pointer"
                          >
                            🗑️ Hapus
                          </button>
                        </div>
                    ...
                    <div
                      v-else
                      class="text-xs text-slate-500 italic p-3 text-center rounded-xl bg-white/5 border border-white/10"
                    >
                      Belum ada foto maskot yang diunggah. Web akan menampilkan foto default
                      (tracon.png, dll).
                    </div>
                  </div>
                  ...
                  <div class="space-y-2">
                    <div class="flex justify-between items-center">
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400"
                        >Kartu Tab Profil Maskot (Asal Nama, Nilai, dll)</label
                      >
                      <button
                        @click="addMascotProfileCard"
                        class="text-[11px] bg-slate-800 hover:bg-slate-900 text-white px-3 py-1.5 rounded-xl font-bold border-0 cursor-pointer transition-all shadow-sm"
                      >
                        ➕ Tambah Kartu Profil
                      </button>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div
                        v-for="(card, cIdx) in siteSettings.tracon_profile_cards"
                        :key="cIdx"
                        class="p-3 rounded-xl relative space-y-2"
                        style="
                          background: rgba(255, 255, 255, 0.06);
                      ...
                      >
                        <button
                          @click="removeMascotProfileCard(cIdx)"
                          class="absolute top-2 right-2 bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/30 px-2 py-0.5 rounded text-[10px] cursor-pointer"
                        >
                          ✕
                        </button>
                  ...
                  <div class="space-y-2 pt-2">
                    <div class="flex justify-between items-center">
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400"
                        >Kartu Tab Filosofi Visual (Helm, Armor, dll)</label
                      >
                      <button
                        @click="addMascotPhilosophyCard"
                        class="text-[11px] bg-slate-800 hover:bg-slate-900 text-white px-3 py-1.5 rounded-xl font-bold border-0 cursor-pointer transition-all shadow-sm"
                      >
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div
                        v-for="(card, cIdx) in siteSettings.tracon_philosophy_cards"
                        :key="cIdx"
                        class="p-3 rounded-xl relative space-y-2"
                        style="
                          background: rgba(255, 255, 255, 0.06);
                      ...
                      >
                        <button
                          @click="removeMascotPhilosophyCard(cIdx)"
                          class="absolute top-2 right-2 bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/30 px-2 py-0.5 rounded text-[10px] cursor-pointer"
                        >
                          ✕
                        </button>
              ...
              <div class="pt-6 border-t border-white/10 space-y-4">
                <div class="flex justify-between items-center">
                  <h4 class="text-sm font-bold text-[#ef4444] uppercase tracking-wider">
                    5. Kemitraan / Klien Kami
                  </h4>
                  <button
                    @click="addClientLogoItem"
                    class="text-xs bg-slate-800 hover:bg-slate-900 text-white px-3.5 py-1.5 rounded-lg border-0 cursor-pointer font-bold transition-all shadow-sm"
                  >
                    ➕ Tambah Klien
                  </button>
                </div>
                ...
                >
                  <div
                    v-if="siteSettings.client_logos && siteSettings.client_logos.length > 0"
                    class="grid grid-cols-1 md:grid-cols-2 gap-4"
                  >
                    <div
                      v-for="(client, cIdx) in siteSettings.client_logos"
                      :key="cIdx"
                      class="p-4 rounded-xl relative space-y-3"
                      style="
                        background: rgba(255, 255, 255, 0.06);
                        border: 1px solid rgba(255, 255, 255, 0.12);
                      "
                    >
                      <button
                        @click="removeClientLogoItem(cIdx)"
                        class="absolute top-2 right-2 bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/30 px-2 py-0.5 rounded text-[10px] cursor-pointer"
                      >
                        ✕ Hapus
                      </button>
                      ...
                      <div>
                        ...
                        <input
                          v-model="client.name"
                          type="text"
                          class="w-11/12 rounded px-3 py-1.5 text-white text-xs focus:outline-none"
                          style="
                            background: rgba(255, 255, 255, 0.08);
                            border: 1px solid rgba(255, 255, 255, 0.15);
                          "
                          placeholder="Contoh: Brantas Abipraya"
                        />
                      </div>
                      ...
                      <div class="space-y-2">
                        ...
                        <div class="flex items-center gap-3">
                          <div
                            class="w-20 h-12 bg-white rounded-lg p-1.5 flex items-center justify-center border border-white/10 shrink-0"
                          >
                            <img
                              v-if="client.image"
                              :src="client.image"
                          ...
                          >
                            ...
                            <input
                              ...
                              class="hidden"
                              accept="image/*"
                              @change="(e) => uploadClientLogo(e, cIdx)"
                            />
                          </label>
                        </div>
                      </div>
                  ...
                  <div
                    ...
                    class="text-xs text-slate-500 italic p-3 text-center rounded-xl bg-white/5 border border-white/10"
                  >
                    Belum ada klien yang ditambahkan. Web utama akan menampilkan mitra default
                    (Brantas Abipraya, Agra Budi).
                  </div>
                </div>
              </div>
              ...
              <div class="pt-6 border-t border-white/10 space-y-4">
                <div class="flex justify-between items-center">
                  <h4 class="text-sm font-bold text-[#ef4444] uppercase tracking-wider">
                    6. Info Kontak & Koordinat Website
                  </h4>
                  <button
                    @click="saveSiteSettings"
                    :disabled="isSavingSettings"
                ...
                >
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      >
                      <input
                        v-model="siteSettings.contact_wa_1_label"
                        type="text"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                          border: 1px solid rgba(255, 255, 255, 0.15);
                        "
                        placeholder="Contoh: WhatsApp Sales 1"
                      />
                    </div>
                    <div>
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >Nomor WhatsApp Sales 1 (Kode Negara tanpa '+', misal: 62813...)</label
                      >
                      <input
                        v-model="siteSettings.contact_wa_1_num"
                        type="text"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                    ...
                    <div>
                      ...
                      >
                      <input
                        v-model="siteSettings.contact_wa_2_label"
                        type="text"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                          border: 1px solid rgba(255, 255, 255, 0.15);
                        "
                        placeholder="Contoh: WhatsApp Sales 2"
                      />
                    </div>
                    <div>
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >Nomor WhatsApp Sales 2</label
                      >
                      <input
                        v-model="siteSettings.contact_wa_2_num"
                        type="text"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                    ...
                    <div>
                      ...
                      >
                      <input
                        v-model="siteSettings.contact_wa_3_label"
                        type="text"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                          border: 1px solid rgba(255, 255, 255, 0.15);
                        "
                        placeholder="Contoh: WhatsApp Sales 3"
                      />
                    </div>
                    <div>
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >Nomor WhatsApp Sales 3</label
                      >
                      <input
                        v-model="siteSettings.contact_wa_3_num"
                        type="text"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                          border: 1px solid rgba(255, 255, 255, 0.15);
                        "
                        placeholder="6287848104524"
                      />
                    </div>
                    <div>
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >Username / Link Instagram</label
                      >
                      <input
                        v-model="siteSettings.contact_instagram"
                        type="text"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                    ...
                    <div>
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >Username / Link TikTok</label
                      >
                      <input
                        v-model="siteSettings.contact_tiktok"
                        type="text"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                    ...
                    <div class="md:col-span-2">
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >Alamat Email Administrasi</label
                      >
                      <input
                        v-model="siteSettings.contact_email"
                        type="email"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
                    ...
                    <div class="md:col-span-2">
                      <label
                        class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1"
                        >URL Google Maps Embed (Src Iframe / Share Link)</label
                      >
                      <textarea
                        v-model="siteSettings.contact_maps_url"
                        rows="3"
                        class="w-full rounded-xl px-3 py-2 text-white text-xs focus:outline-none resize-none"
                        style="
                          background: rgba(255, 255, 255, 0.08);
        ...
        <q-dialog v-model="showProductModal" persistent max-width="800px">
          <q-card
            class="bg-white text-slate-800 border border-slate-200 rounded-3xl w-full max-w-[800px] overflow-hidden shadow-2xl"
          >
            <q-card-section
              class="bg-slate-50 p-5 flex justify-between items-center border-b border-slate-200"
            >
            ...
            <q-card-section class="p-6 overflow-y-auto max-h-[70vh] space-y-6">
              ...
              <div class="flex gap-2 p-1 bg-slate-100 border border-slate-200 rounded-xl mb-4">
                <button
                  ...
                  :class="
                    dialogTab === 'general'
                      ? 'bg-[#d21d1d] text-white shadow-sm'
              ...
              <div v-show="dialogTab === 'general'" class="space-y-4">
                <div class="grid grid-cols-3 gap-4">
                  ...
                  <div class="col-span-2">
                    ...
                    <input
                      ...
                      :class="
                        showValidationErrors && !currentProduct.name
                          ? 'border-red-500 focus:border-red-500'
                          : 'border-slate-200 focus:border-[#d21d1d]'
                      "
                      placeholder="Nama produk precast..."
                    />
                    <span
                      v-if="showValidationErrors && !currentProduct.name"
                      class="text-red-500 text-xs font-semibold mt-1.5 block"
                    >
                ...
                <div>
                  <label
                    class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2"
                    >Foto Utama Produk <span class="text-red-500">*</span></label
                  >
                  ...
                  <div v-if="currentProduct.image" class="mb-3 relative group inline-block">
                    ...
                    <button
                      type="button"
                      @click="currentProduct.image = ''"
                      class="absolute top-2 right-2 bg-red-500 text-white w-6 h-6 rounded-full text-xs font-bold opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer border-0"
                    >
                      ✕
                    </button>
                  </div>
                  ...
                  <div class="flex gap-2">
                    <input
                      ...
                      :class="
                        showValidationErrors && !currentProduct.image
                          ? 'border-red-500 focus:border-red-500'
                          : 'border-slate-200 focus:border-[#d21d1d]'
                      "
                    ...
                    />
                    <label
                      class="flex items-center gap-2 px-4 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl cursor-pointer font-bold text-xs transition-all shadow-sm whitespace-nowrap"
                      :class="{ 'opacity-60 pointer-events-none': isUploadingMain }"
                    >
                      ...
                      <input
                        ...
                        class="hidden"
                        accept="image/*"
                        @change="(e) => uploadMainImage(e)"
                        :disabled="isUploadingMain"
                      />
                    </label>
                  </div>
                  <span
                    v-if="showValidationErrors && !currentProduct.image"
                    class="text-red-500 text-xs font-semibold mt-1.5 block"
                  >
                    ⚠️ Foto utama produk wajib diupload atau diisi URL gambarnya!
                  </span>
                  <p v-else class="text-[10px] text-slate-400 mt-1.5">
                    Pilih file (JPG/PNG/WEBP) atau tempel URL langsung
                  </p>
                </div>
                  ...
                  <div class="flex justify-between items-center mb-2">
                    ...
                    <div class="flex gap-2">
                      <label
                        ...
                        :class="{ 'opacity-60 pointer-events-none': isUploadingGallery }"
                      >
              ...
              <div v-show="dialogTab === 'sizes'" class="space-y-6">
                ...
                <div>
                  <div class="flex justify-between items-center mb-3">
                    <label class="text-xs font-bold text-slate-600 uppercase tracking-wider"
                      >Opsi Ukuran / Dimensi (Simulasi Tonase)</label
                    >
                    <button
                      type="button"
                      @click="addSizeOption"
                  ...
                  <div class="space-y-3">
                    <div
                      v-for="(opt, idx) in currentProduct.size_options"
                      :key="idx"
                      class="flex flex-col sm:flex-row gap-3 p-3 bg-slate-50 border border-slate-200 rounded-2xl relative"
                    >
                      <div class="flex-1">
                ...
                <div>
                  ...
                  <div class="space-y-3">
                    <div
                      v-for="(feat, idx) in currentProduct.features"
                      :key="idx"
                      class="flex flex-col gap-2 p-3 bg-slate-50 border border-slate-200 rounded-2xl relative"
                    >
                      <div>
                        <label class="block text-[10px] text-slate-500 font-semibold mb-1"
                          >Judul Keunggulan</label
                        >
                        <input
                          v-model="feat.title"
                          type="text"
            ...
            <!-- Save Button -->
            <q-card-actions
              class="bg-slate-50 p-4 border-t border-slate-200 flex justify-end gap-3"
            >
              <button
                @click="showProductModal = false"
...
async function uploadMainImage(event) {
  ...
  try {
    ...
    $q.notify({
      color: 'positive',
      position: 'top',
      message: '✅ Foto utama berhasil diupload!',
      icon: 'check_circle',
    })
  } catch (err) {
    ...
    if (err.message?.includes('bucket') || err.message?.toLowerCase().includes('not found')) {
      userMessage =
        '❌ Bucket "product-images" belum dibuat di Supabase Anda! Buka Supabase Dashboard -> Storage -> Create New Bucket bernama "product-images" (atur ke Public).'
    } else if (
      err.status === 403 ||
      ...
      userMessage =
        '❌ RLS Blocked: Akses upload ditolak! Buka Supabase Storage -> bucket "product-images" -> Policies -> Tambahkan Policy (Allow Insert/Select untuk anonymous/public).'
    }
  ...
  if (error) {
    ...
    siteSettings.value = {
      ...
      hero_slides: safeParse(data.hero_slides, [
        {
          image: 'precast_concrete_factory_1782379014858.png',
          title: 'Beton Precast Mutu Tinggi',
          desc: 'Menghadirkan kekuatan optimal dan presisi tinggi untuk segala kebutuhan infrastruktur.',
        },
      ]),
      ...
      portfolio_items: safeParse(data.portfolio_items, [
        {
          ...
          title: 'Pemasangan U-Ditch Tol Cipali',
          desc: 'Suplai dan pemasangan U-Ditch ukuran 60x60 cm sepanjang 2.4 km untuk drainase utama jalan tol.',
          image: 'precast_concrete_uditch_1782378967095.png',
        },
        {
          category: 'Saluran Irigasi',
          title: 'Saluran Box Culvert Primer Bekasi',
          desc: 'Konstruksi gorong-gorong penyaluran air pemukiman menggunakan Box Culvert ukuran 120x120 cm.',
          image: 'precast_concrete_structure_1782378994309.png',
        },
        {
          category: 'Pondasi Berat',
          title: 'Pondasi Spun Pile Pelabuhan Patimban',
          desc: 'Pemancangan tiang pancang spun pile silinder diameter 500 mm untuk dermaga logistik laut.',
          image: 'precast_concrete_factory_1782379014858.png',
        },
      ]),
      tracon_images: safeParse(data.tracon_images, [
        'tracon.png',
        'tracon2.png',
        'tracon3.png',
        'tracon4.png',
      ...
      tracon_profile_cards: safeParse(data.tracon_profile_cards, [
        {
          title: 'Asal Nama TRACON',
          desc: 'Singkatan dari <strong>TRA</strong>nsformer <strong>CON</strong>crete. Melambangkan robot pelindung dan pembangun konstruksi beton yang kokoh.',
        },
        {
          title: 'Nilai Kerja TRACON',
          desc: 'Mengutamakan presisi tinggi (Precision), kekuatan struktural (Strength), ketahanan 100 tahun (Durability), dan keselamatan kerja (Safety).',
        },
      ]),
      tracon_philosophy_cards: safeParse(data.tracon_philosophy_cards, [
        {
          icon: '🪖',
          title: 'Helm Safety Merah',
          desc: 'Simbol kepemimpinan, keberanian, keselamatan kerja utama di lapangan proyek.',
        },
        {
          icon: '🛡️',
          title: 'Armor Baja K-500',
          desc: 'Kekuatan tekan beton precast mutu K-500 ke atas yang tahan segala cuaca.',
        },
        {
          icon: '⚡',
          title: 'Inti Reaktor Energi',
          desc: 'Melambangkan energi efisiensi tinggi, percepatan fabrikasi, dan pengiriman tepat waktu.',
        },
      ]),
      ...
      tracon_faq: safeParse(data.tracon_faq, [
        {
          question: 'Berapa minimal order beton precast?',
          answer:
            'Minimal pemesanan tergantung jenis produknya bro. Untuk U-Ditch dan Box Culvert biasanya minimal 10-20 pcs, sedangkan pipa beton (RCP) dan spun pile disesuaikan dengan volume proyek. Hubungi sales kami untuk negosiasi kuantitas kecil!',
        },
        {
          question: 'Apakah produk CITRACon sudah berstandar SNI?',
          answer:
            'Tentu saja! Semua produk beton precast CITRACon diproduksi dengan presisi tinggi menggunakan mutu beton minimal K-350 hingga K-500, dan sudah tersertifikasi Standar Nasional Indonesia (SNI) untuk ketahanan jangka panjang.',
        },
        {
          question: 'Bagaimana sistem pengiriman produk?',
          answer:
            'Kami menggunakan armada logistik mandiri (truk tronton/trailer) untuk mengirimkan produk langsung ke lokasi proyek Anda di seluruh area Jawa Barat, DKI Jakarta, Banten, dan sekitarnya. Pastikan akses jalan proyek cukup untuk bilewati kendaraan berat kami.',
        },
        {
          question: 'Apakah bisa memesan ukuran kustom (Custom Moulding)?',
          answer:
            'Sangat bisa bro! CITRACon memiliki pabrik dengan fasilitas cetakan khusus yang fleksibel. Jika proyek Anda membutuhkan dimensi U-Ditch, Box Culvert, atau precast panel non-standar, tim engineering kami siap memfabrikasi cetakan kustom sesuai SPK.',
        },
        {
          question: 'Bagaimana sistem pembayaran pemesanan?',
          answer:
            'Pembayaran biasanya dilakukan melalui transfer bank ke rekening resmi PT CITRACon Beton Precast sesuai termin kesepakatan Surat Perjanjian Kerja (SPK). Kami mendukung opsi pembayaran bertahap (Down Payment + Termin progress fabrikasi & pengiriman) maupun lunas di awal proyek.',
        },
        {
          question: 'Berapa lama estimasi proses fabrikasi produk?',
          answer:
            'Proses produksi berkisar antara 3 hingga 7 hari kerja tergantung volume pesanan dan antrean cetakan di pabrik kami. Untuk produk kustom (cetakan non-standar), estimasi waktu pengerjaan akan disesuaikan dengan kebutuhan pembuatan cetakan (moulding) baru oleh tim engineering kami.',
        },
        {
          question: 'Di mana lokasi pabrik batching plant CITRACon?',
          answer:
            'Batching plant dan pabrik utama kami berlokasi sangat strategis di area industri dengan akses jalan yang memadai untuk memudahkan mobilisasi material & armada pengiriman alat berat. Informasi peta navigasi Google Maps lengkap dapat Anda akses pada section Hubungi Kami di bagian bawah website.',
        },
      ]),
...
async function uploadSlideImage(event, sIdx) {
  ...
  try {
    ...
    $q.notify({
      color: 'positive',
      position: 'top',
      message: '✅ Gambar banner berhasil diupload!',
      icon: 'check_circle',
    })
  } catch (err) {
...
async function uploadMascotImage(event) {
  ...
  try {
    ...
    $q.notify({
      color: 'positive',
      position: 'top',
      message: '✅ Foto maskot berhasil diupload!',
      icon: 'check_circle',
    })
  } catch (err) {
...
function resetFaqToDefault() {
  if (
    confirm(
      'Apakah Anda yakin ingin menyetel ulang daftar FAQ ke default? Semua FAQ kustom Anda akan terhapus. (Ingat untuk mengklik Simpan setelah melakukan reset ini!)',
    )
  ) {
    siteSettings.value.tracon_faq = [
      {
      ...
      },
      {
        question: 'Bagaimana sistem pengiriman produk?',
        answer:
          'Kami menggunakan armada logistik mandiri (truk tronton/trailer) untuk mengirimkan produk langsung ke lokasi proyek Anda di seluruh area Jawa Barat, DKI Jakarta, Banten, dan sekitarnya. Pastikan akses jalan proyek cukup untuk dilewati kendaraan berat kami.',
      },
      {
        question: 'Apakah bisa memesan ukuran kustom (Custom Moulding)?',
        answer:
    ...
    $q.notify({
      color: 'positive',
      position: 'top',
      message: '✅ FAQ di-reset ke default! Jangan lupa klik SIMPAN di bawah!',
      icon: 'check_circle',
    })
  }
}
...
async function uploadPortfolioImage(event, pIdx) {
  ...
  try {
    ...
    $q.notify({
      color: 'positive',
      position: 'top',
      message: '✅ Gambar proyek berhasil diupload!',
      icon: 'check_circle',
    })
  } catch (err) {
...
async function uploadClientLogo(event, cIdx) {
  ...
  try {
    ...
    $q.notify({
      color: 'positive',
      position: 'top',
      message: '✅ Logo klien berhasil diupload!',
      icon: 'check_circle',
    })
  } catch (err) {
