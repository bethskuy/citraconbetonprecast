  ...
  <q-page class="q-pb-xl bg-[#f8fafc] text-slate-800">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 flex items-center gap-4 mb-6">
      ...
      <nav class="flex items-center gap-2 text-xs sm:text-sm text-slate-500">
        <router-link
          to="/"
          class="hover:text-[#d21d1d] transition-colors flex items-center gap-1 font-medium no-underline"
        >
          🏠 Beranda
        </router-link>
        <span>/</span>
    ...
    <div
      v-if="isLoading"
      class="max-w-7xl mx-auto px-4 py-32 flex flex-col items-center justify-center text-center"
    >
      <div class="relative w-24 h-24 mb-6">
        <!-- Outer Glowing Spin Ring -->
        <div
          class="absolute inset-0 rounded-full border-4 border-[#0b1c3f]/10 border-t-[#d21d1d] animate-spin"
        ></div>
        <!-- Inner CITRACon logo pulsing -->
        <div
          class="absolute inset-4 rounded-2xl bg-white border border-slate-100 flex items-center justify-center shadow-lg shadow-slate-200/50 animate-pulse"
        >
          <img
      ...
      <p class="text-xs text-slate-500 mt-1 max-w-xs leading-relaxed">
        Menyiapkan spesifikasi teknis dan formulir penawaran terbaik untuk Anda...
      </p>
    </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-8 space-y-8">
          ...
          <div class="relative rounded-3xl overflow-hidden shadow-md h-80 sm:h-96 md:h-[450px]">
            ...
            <div class="absolute bottom-8 left-8 right-8 text-white z-10">
              <span
                class="px-3.5 py-1.5 bg-[#d21d1d] text-white font-bold text-xs uppercase tracking-wider rounded-full shadow-sm"
              >
                {{ product.icon }} Beton Pracetak
              </span>
              <h1
                class="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white mt-4 uppercase tracking-tight"
              >
                {{ product.name }}
              </h1>
            </div>
          ...
          <div
            ...
            class="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm"
          >
            <h3 class="text-sm font-bold uppercase tracking-wider text-[#0b1c3f] mb-4">
              Galeri Foto Produk
            </h3>
            <div class="flex flex-wrap gap-4">
              <div
                ...
                :class="
                  activeImage === img
                    ? 'border-[#d21d1d] scale-105 shadow-md shadow-red-500/10'
          ...
          <div class="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            <h2
              class="text-xl sm:text-2xl font-bold text-[#0b1c3f] border-b border-slate-100 pb-4 mb-4"
            >
              Deskripsi Produk
            </h2>
          ...
          <div class="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm">
            <h2
              class="text-xl sm:text-2xl font-bold text-[#0b1c3f] border-b border-slate-100 pb-4 mb-6"
            >
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div
                ...
                :class="[
                  idx % 4 === 0
                    ? 'bg-gradient-to-br from-[#0b1c3f] via-[#1e1b4b] to-[#3b0764] hover:shadow-indigo-500/30'
                    : idx % 4 === 1
                      ? 'bg-gradient-to-br from-[#d21d1d] via-[#701a75] to-[#3b0764] hover:shadow-fuchsia-500/30'
                      : idx % 4 === 2
                        ? 'bg-gradient-to-br from-[#d97706] via-[#7c2d12] to-[#1e1b4b] hover:shadow-amber-500/30'
              ...
              >
                ...
                <span
                  class="text-xl transition-transform duration-300 group-hover:scale-125 group-hover:rotate-12 inline-block relative z-20"
                >
          ...
          <div class="bg-white rounded-3xl border border-slate-100 p-8 shadow-sm overflow-hidden">
            ...
            <div class="overflow-x-auto">
              <div
                v-if="!product.technical_specs && !product.technicalSpecs"
              ...
              <table v-else class="w-full text-left text-xs sm:text-sm border-collapse">
                <thead>
                  <tr
                    class="bg-[#0b1c3f] text-white uppercase text-[10px] sm:text-xs tracking-wider"
                  >
                ...
                <tbody class="divide-y divide-slate-100">
                  ...
                  >
                    <td class="p-4 font-bold text-[#0b1c3f] capitalize">
                      {{ name.replace(/_/g, ' ') }}
                    </td>
        ...
        <div class="lg:col-span-4 lg:sticky lg:top-8 space-y-6">
          ...
          <div class="bg-white rounded-3xl border border-slate-100 p-8 shadow-md">
            <div class="text-xs font-bold uppercase tracking-wider text-[#d21d1d] mb-2">
              Simulasi Kebutuhan
            </div>
            ...
            <div class="space-y-4">
              ...
              <div class="relative">
                ...
                >
                  <span class="text-xs sm:text-sm">{{
                    currentSize
                      ? currentSize.label
                      : product.size_options?.length || product.sizeOptions?.length
                        ? 'Pilih Ukuran'
                        : 'Ukuran tersedia (hubungi sales)'
                  }}</span>
                  <q-icon
                    name="expand_more"
              ...
              <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2"
                  >Alamat Kirim / Lokasi Proyek</label
                >
                <textarea
                  v-model="address"
                  rows="2"
              ...
              <div
                v-if="currentSize"
                class="bg-slate-50 rounded-2xl p-4 border border-slate-100 text-xs sm:text-sm space-y-2"
              >
                <div class="flex justify-between">
                  <span class="text-slate-500 font-medium">Mutu Kuat Tekan:</span>
                  <span class="font-bold text-[#0b1c3f]">{{
                    product.specs ? product.specs.split('/')[0].trim() : '-'
                  }}</span>
                </div>
                ...
                <div class="flex justify-between" v-if="currentSize.weight && quantity > 0">
                  ...
                  <span class="font-extrabold text-[#059669]"
                    >{{ ((currentSize.weight * quantity) / 1000).toFixed(2) }} Ton</span
                  >
                </div>
              </div>
          ...
          <div
            ...
            class="bg-white rounded-3xl border border-slate-100 p-6 shadow-md"
          >
            <div class="text-xs font-bold uppercase tracking-wider text-[#d21d1d] mb-1">
              Pilihan Produk
            </div>
            <h3 class="text-base font-bold text-[#0b1c3f] mb-4 border-b border-slate-100 pb-3">
              Katalog Precast
            </h3>
            <div class="space-y-2">
              <router-link
                ...
                :class="
                  p.id === product.id
                    ? 'bg-[#0b1c3f]/5 border-[#0b1c3f] text-[#0b1c3f]'
              ...
              >
                <div class="flex items-center gap-2.5">
                  ...
                  <span
                    class="uppercase tracking-wide font-extrabold group-hover:text-[#d21d1d] transition-colors"
                    >{{ p.name }}</span
                  >
                </div>
                <q-icon
          ...
          <!-- Trust Badges Panel -->
          <div
            class="bg-gradient-to-br from-[#0b1c3f] to-[#122b5e] text-white rounded-3xl p-8 shadow-sm"
          >
            <h4
              class="font-bold text-base mb-4 uppercase tracking-wide border-b border-white/10 pb-3"
            >
              Standar Kualitas
            </h4>
            <div class="space-y-4">
      ...
      <div v-if="products && products.length" class="mt-16 pt-10 border-t border-slate-200/50">
        <div class="text-center max-w-2xl mx-auto mb-10">
          ...
          <h2 class="text-2xl md:text-3xl font-bold text-[#0b1c3f] tracking-tight uppercase">
            Produk Precast Terkait
          </h2>
          <p class="text-slate-500 text-sm mt-2">
            Telusuri katalog produk pracetak berkualitas tinggi kami lainnya
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          ...
          >
            ...
            <div class="relative rounded-xl overflow-hidden h-40 mb-4 bg-slate-100">
              ...
              />
              <div
                class="absolute top-3 left-3 bg-[#0b1c3f] text-white w-8 h-8 rounded-full flex items-center justify-center text-sm shadow"
              >
                {{ p.icon }}
              </div>
            </div>
            ...
            <div class="flex-grow flex flex-col justify-between">
              <div>
                <h3
                  class="text-sm font-extrabold text-[#0b1c3f] uppercase tracking-wider mb-2 group-hover:text-[#d21d1d] transition-colors"
                >
                  {{ p.name }}
                </h3>
                <p class="text-slate-500 text-[11px] leading-relaxed line-clamp-3 mb-4">
                  {{
                    p.desc ||
                    'Beton precast bermutu tinggi berstandar SNI untuk infrastruktur berkualitas.'
                  }}
                </p>
              </div>
    ...
    <div v-else class="max-w-7xl mx-auto px-4 py-20 text-center">
      ...
      <router-link
        to="/"
        class="mt-6 inline-block bg-[#0b1c3f] text-white font-bold px-6 py-2.5 rounded-full shadow-md"
      >
        Kembali ke Beranda
      </router-link>
    </div>
      ...
      >
        <q-card-section class="q-pt-none text-center">
          ...
          <p class="text-slate-600 text-sm">
            Silakan isi data Anda, tim spesialis beton kami akan segera menghubungi Anda dengan
            penawaran detail.
          </p>
      ...
      >
        <q-card-section class="q-pt-none text-center relative overflow-hidden">
          ...
          <p class="text-slate-300 text-xs mb-6">
            Silakan pilih salah satu layanan WhatsApp Sales kami untuk mengirim penawaran:
          </p>
            ...
            >
              ...
              <div class="text-left">
                <div
                  class="text-sm font-bold text-white group-hover:text-[#d21d1d] transition-colors"
                >
                  {{ siteSettings.contact_wa_3_label || 'WhatsApp Sales 3' }}
                </div>
...
async function fetchProduct() {
  ...
  const fetchQuery = supabase.from('products').select('*').eq('id', id).single()
  ...
  if (product.value) {
    ...
    if (metaDesc) {
      metaDesc.setAttribute(
        'content',
        `Pabrik ${product.value.name} beton precast berkualitas tinggi standar SNI. Melayani pengiriman ke seluruh Jawa. Hubungi kami untuk penawaran harga terbaik!`,
      )
    }
  }
})
